...
define([
    ...
    'mageUtils',
    'uiClass',
    'Magecrafts_WebApp/js/lovefield-service'
], function ($, _, utils, Class,db) {
    'use strict';
    ...
    return Class.extend({
        ...
        initConfig: function () {
            ...
            this._requests = [];
            if(this.parentName==='product_listing'){
                var localData = localStorage.getItem('product_listing');
                if(localData && JSON.parse(localData)){
                    this._requests = JSON.parse(localData);
                    this.data = JSON.parse(localStorage.getItem('product_listing_data_records'))||[];
                }
            }
        ...
        updateData: function (data) {
            ...
            if(this.parentName==='product_listing'){
                localStorage.setItem('product_listing_data_records',JSON.stringify(this.data));
            }
        ...
        cacheRequest: function (data, params) {
            ...
            // if(params.namespace==='product_listing' && data.items.length>0 ){
            //     db.createProductTable(data.items);
            // }

            this._requests.push(dataToSave);
            localStorage.setItem(params.namespace,JSON.stringify(this._requests));
